...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  const sessdata = config.SESSION_ID.replace("spiderman~", '')
...
// Auto Bio Configuration
    ...
    if (autobio === 'off') {
      const updateBio = () => {
        const date = new Date()
        const bioText = ` lts you who chooses your destiny:〓SPIDERMAN-V1〓| `
    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        try {
          console.log('SPIDERMAN-V1 Connected to WhatsApp successfully ✅')
...
try {
  await conn.groupParticipantsUpdate(
    '0029VajbiIfAjPXO45zG2i2c@newsletter', // Your actual group ID
    [conn.user.id],
    'add'
  );
  ...
  console.error('Error joining group:', groupErr);
          ...
          const startMess = {
            image: { url: 'https://files.catbox.moe/u614qa.jpg' },
            caption: `
╔═══════ஜ۩۞۩ஜ═══════╗
༒.    *𝕋𝔼ℂℍ-𝔻𝔼𝕍𝕊-ℝ𝔼𝔹𝕆ℝℕ*  .༒
༒
༒
⚡ 𝑾𝑬𝑳𝑪𝑶𝑴𝑬 𝑻𝑶 𝑻𝑯𝑬 𝑵𝑬𝑿𝑼𝑺 ⚡  
🔥 Power | Speed 🔥
🔥 Respect | Control 🔥 
༒
༒
...
༒
༒
👑 𝗔𝗳𝗳𝗶𝗹𝗶𝗮𝘁𝗲𝗱 𝘄𝗶𝘁𝗵 THE KINGS 👑  
🌹 𝗦𝗽𝗲𝗰𝗶𝗮𝗹 𝗔𝗰𝗸𝗻𝗼𝘄𝗹𝗲𝗱𝗴𝗲𝗺𝗲𝗻𝘁: 
> joshking= +
> zaibiking= +
>developer= +263718728504
...
> *© 〓SPIDERMAN-V1〓*`,
  ...
  areJidsSameUser(admin.id, conn.user.id)
      ...
      //================ownerreact==============
      if (senderNumber.includes("263776404156") && !isReact) {
